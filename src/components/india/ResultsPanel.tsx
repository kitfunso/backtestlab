    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-4 sm:grid-cols-7 gap-1.5 mt-3">
        {[
          ...
          { label: 'Profit Factor', value: result.metrics.profit_factor.toFixed(2), color: result.metrics.profit_factor >= 1 ? '#22C55E' : '#EF4444' },
...
function MetricsTab({ result, isLight }: { result: BacktestResult; isLight: boolean }) {
  ...
  const cards: { label: string; value: string; color: 'positive' | 'negative' | 'accent' | 'neutral' }[] = [
    ...
    { label: 'Profit Factor', value: m.profit_factor.toFixed(2), color: m.profit_factor >= 1 ? 'positive' : 'negative' },
